...
(function(library){
		...
		*/  
        library.equity=function(instrument){
            ...
            if(null===this.quantity) throw new Error("equity: must provide valid quantity");
        };
		/**
...
//    this.current_accrued_interest = instrument.current_accrued_interest || 0;
    ...
    // repay schedule
    this.is_amortizing = (this.repay_amount.reduce(function(a, b) {
  ...
  library.fixed_income.prototype.fair_rate_or_spread = function(disc_curve, spread_curve, fwd_curve) {
    ...
    var res = outstanding_principal - library.dcf({
        date_pmt: c.date_pmt,
        t_pmt: c.t_pmt,
        pmt_total: pmt
      },
...
;(function(library) {
    ...
    library.get_safe_surface = function(surface) {
		...
		if(typeof surface._rule === 'object'){
			...
			if (surface._rule.model==="additive") surface.get_rate=function(t_expiry,t_term){
					return get_surface_rate(surface,t_expiry,t_term) + get_surface_rate(scen,t_expiry,t_term);
				};
			if (surface._rule.model==="absolute") surface.get_rate=function(t_expiry,t_term){
		...
		if (surface.moneyness && surface.smile) {
			if (!surface.moneyness.length || !surface.smile.length) throw new Error("get_cube_rate: invalid cube, moneyness and smile must be arrays");
    ...
    function get_slice_rate(surface, i_expiry, t_term) {
        ...
        if (tmax - tmin < 1 / 512) throw new Error("get_slice_rate: invalid surface, support points must be increasing and differ at least one day");
        var temp = 1 / (tmax - tmin);
	...
	function get_surface_rate(surface, t_expiry, t_term) {
        ...
        // interpolation (linear)
        if (tmax - tmin < 1 / 512) throw new Error("get_surface_rate: invalid surface, support points must be increasing and differ at least one day");
        var temp = 1 / (tmax - tmin);
    ...
    library.get_surface_rate = function(surface, t_expiry, t_term) {
        if (surface.get_rate instanceof Function) return surface.get_rate(t_expiry, t_term);
